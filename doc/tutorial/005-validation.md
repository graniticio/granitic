# Tutorial - Validating data submitted to a web service
...
At the moment, we can only submit the artist's name and age in as part of that web service call.
...
```
Rules are expressed as a JSON array of string arrays. Each entry in the top level array maps to a field that we want to 
validate on the our web service endpoint's [unmarshalling target](https://godoc.org/github.com/graniticio/granitic/ws/handler#WsUnmarshallTarget) in this
...
the validation _operations_ that should be applied to the field. In this cause we asking that a single operation is applied - the <code>REQ</code> operation
means the field is required and validation will fail if it is _not provided in the JSON body_ (this is different to the field being present but empty)
...
<code>resources/components/components.json</code> file. We'll need to modify this component's configuration and also declare a new one:
...
<code>"Rules": "conf:submitArtistRules"</code>
...
so you'll have to amend the <code>pacakages</code> array at the top of the file to include:

<code>"github.com/graniticio/granitic/validate"</code>
...
If you haven't already done so, please look at the _Testing POST services_ section in the [previous tutorial](004-data-capture.md)
...
and the HTTP status code will be <code>400 Bad Request</code> You can see that Granitic shows which fields errors were
...
```
 ...
 This <code>TRIM</code> operation does not permanently alter the data that is submitted to your service. If you submit <code>" ABCD "</code> 
 ...
 a second operation <code>HARDTRIM</code> that permanently trims the supplied data.
 ...
 the optionally supplied age is in a reasonable range and change the first rule so that only letters and spaces are allowed in names:
 
 ```json
    ["Name",  "STR",  "REQ:NAME_MISSING", "TRIM", "LEN:5-50:NAME_BAD_LENGTH", "REG:^[A-Z]| +$:NAME_BAD_CONTENT"],
    ["Age",   "INT",  "RANGE:0|150:AGE_INVALID"]
 ```
 ...
 We've added a second rule to make sure that <code>Age</code> is in the range 1 to 150 (note the pipe separator). We've also added two new error codes, so add the following
 messages to your <code>serviceErrors</code>:
 
 ```json
  ["C", "NAME_BAD_CONTENT", "Names can only contain letters and spaces."],
  ["C", "AGE_INVALID", "Age must be in the range 0-150"]
```
...
if the <code>LEN</code> operation has already failed. Change the rule to:
 ...
 Sometimes a check is so fundamental that there is no point processing any further rules. Change the rule to:
 
 ```json
    ["Name",  "STR",  "REQ:NAME_MISSING", "TRIM", "STOPALL", "LEN:5-50:NAME_BAD_LENGTH", "BREAK", "REG:^[A-Z]| +$:NAME_BAD_CONTENT"]
```
 
and notice that the <code>Age</code> is no longer executed if any problems are found with <code>Name</code>
